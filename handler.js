        ...
        try {
            ...
            try {
             ...
             if (!isNumber(user.ayam)) user.ayam = 0
                    ...
                    if (!isNumber(user.lastclaimlimit)) user.lastclaimlimit = 0
            ...
            if (opts['gconly'] && !m.fromMe && !m.chat.endsWith('g.us') && !global.db.data.users[m.sender].premium) return
            ...
            for (let name in global.yuhao) {
                let plugin = global.yuhao[name]
                if (!plugin) continue
            ...
            for (let name in global.yuhao) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.reply(m.chat, `Limit anda habis, silahkan ambil dengan cara mengetik *.claimlimit*`, m)
            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        let nckgc = await conn.getName(id) 
                         let usr = `@${user.split('@')[0]}`
...
/*const message = { 
            document: { url: thumb },
            jpegThumbnail: await conn.resize(pp, 150, 150),
            fileName: nckgc,
            mimetype: 'image/png',
            caption: text,
            contextInfo: {
  ...
  showAdAttribution: false,
	title: action == 'add' ? 'Welcome to ' + nckgc : 'Leaving From ' + nckgc,
	body: wm,
	thumbnailUrl: action == 'add' ? 'https://telegra.ph/file/7f6b8e88334f1693681cf.jpg' : 'https://telegra.ph/file/0c9aec115c9fe67facb99.jpg',
	sourceUrl: sgc,
	mediaType: 1,
	renderLargerThumbnail: true
	}}
        }
    await conn.sendMessage(id,message, { quoted:null})*/
